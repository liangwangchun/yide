...
//                        couponsDetail(cartsId, coupon);//优惠卷
            	...
            	couponsDetail(cartIds, couponNo);//优惠卷
...
function couponList(totalCount, couponidx) {
    ...
    $('#coupons' + params.tmn).find("li").on("click", function () {
        ...
        couponsDetail(cartIds,couponNo);
...
function findTmn(tmn, countPrice,couponAmt) {
	...
	var price = freeCount - 0 - countPriceOnion + rebatePrice;
 ...
 * @param coupon
 */
function couponsDetail(cartsId, coupon) {
	console.log(coupon);
	console.log(JSON.stringify(cartsId));
	 $.ajax({
	        type: "POST",
	        ...
	        success: function (data) {
	        	...
	        	alert(couponAmt);
	        	findTmn(tmn, countPrice,couponAmt - discountAmt)
	        }
	    });
