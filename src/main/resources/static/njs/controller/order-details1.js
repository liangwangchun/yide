 ...
 * @returns
 */
function jumpToResale(id, sodId, sodNo, deliveryTime, createTime, expressNo) {
    var nowTime = toGetTime("" + new Date().getTime(), true);
    var flag = false;
    if (deliveryTime != "undefined") {
        deliveryTime = toGetTime(deliveryTime, false);
        if (parseInt(nowTime) - deliveryTime > 3600000 * 24 * 10) {
            flag = true;
            sessionStorage.type = 1;
        }
    }
    if (createTime != "undefined") {
        createTime = toGetTime(createTime, false);
        if (parseInt(nowTime) - createTime > 3600000 * 24 * 30 && (expressNo == "" || expressNo == "undefined")) {
            flag = true;
