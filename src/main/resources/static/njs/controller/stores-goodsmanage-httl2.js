     ...
     $(".doubtBtn").on("click",function () {
         ...
         mBox.open({
            ...
            // closeBtn: [true, 1],
             ...
             maskClose: false,
             success:function () {
             	$("#leCostView").text(dval[1]);
             	$("#reCostView").text(dval[0]);
...
//         $(this).parent().prev("div").find("a").attr("href","../wx/foreign-detail.html?id="+dval[8]+"&tmn="+tmn);//
         ...
         $(this).parent().prev("div").find("p").eq(1).find("i").html("¥"+jems.formatNum((dval[4]*0.7),2));//用户服务收入
//         $(this).parent().prev("div").find("p").eq(2).find("span").html("¥"+jems.formatNum(dval[4]*0.5,2));//zlr
         $(this).parent().prev("div").find("p").eq(2).find("span").html("¥"+jems.formatNum(parseFloat(dval[4])*0.3,2)+"("+jems.formatNum(parseFloat(dval[4])*0.3/dval[2]*100,2)+"%)");//店铺服务收入
         $(this).parent().prev("div").find("p").eq(1).attr("data-cost","¥"+jems.formatNum(dval[4]*0.5,2)+",¥"+jems.formatNum(dval[4]*0.2,2));
     });
    toggleSortStyle();
...
function sortByType(sortType,upordown){
    ...
    var url = "mygoodsviewV3?flag="+param.flag+"&ordertype="+sortType+"&upordown="+upordown;
